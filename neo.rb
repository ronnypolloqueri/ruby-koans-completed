...
module Neo
  ...
  class Sensei
    ...
    def instruct
      if failed?
        _guide  = indent(failure.message).join
        _errors = indent(find_interesting_lines(failure.backtrace)).join
        _number = _errors.match(/\d+/).to_s
        Libnotify.show(:body => "#{_errors}", 
                       :summary => "#{_number} :#{_guide}", 
                       :timeout => 7.5,
